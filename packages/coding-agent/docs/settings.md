...
`manage_subagent` supports `inspect`, `extend`, `stop`, owner-bound `follow_up`, `resume`, and `delete`. Follow-up requires a stable `requestId`, is deduplicated, queues through the retained native Ice child, preserves its original scope/tools/model/budgets, and is rejected while a user has Take Control. `resume` continues an eligible completed child in its original session under a new run id, re-derives profile/resources/project trust/tool authority fail-closed at the resume boundary, re-points the session's own tool/stream/turn-stop wrappers at the resumed run's hooks, authority callback, and budgets, and cannot widen model/profile/scope/tools; `delete` is owner-scoped and idempotent, rejects active children, and releases both the retained session and its historical view snapshot. Durable async acceptance stores the resolved thinking, timeout, turn/tool/output budgets, tools, and profile source hash; settings changes do not silently re-resolve accepted jobs.
