...
- Removed active aggregate token-budget configuration and enforcement for delegated children and batches. `maxTotalTokens` and `totalTokenBudget` inputs are rejected explicitly; legacy persisted contract/result budget fields are discarded during migration. Provider usage remains observational telemetry, while bounded turn, tool-call, timeout, and output-byte controls remain supported.
...
- Added bounded child transient retry inheritance and lifecycle observability, deterministic wall/turn/tool wrap-up ramp-down, and redacted local spill artifacts for oversized partial or final reports while keeping parent-facing results capped.
...
- Added bounded subagent timeout multiplexing. `delegate` accepts an optional `background: true` control option that returns a retained managed handle as soon as the child is admitted, so a long foreground child no longer blocks the parent tool call; the same run ID, child session, model, profile, scope, and tool authority continue with one initial task prompt and no replacement child. `manage_subagent` gains `peek` (immediate observational snapshot), `wait` (bounded `waitMs`, at most 60000 ms, defaulting to 30000 ms), and `detach` (retain the same child so the parent can continue other work: a running child is retained in place with no budget change, and a paused child resumes under a separate bounded retention pool instead of spending the extension reserve). Management wait expiry is reported as `waitExpired` and never marks the child `timed_out`; managed terminal results are published exactly once into a bounded retained-result ledger before the terminal transition wakes pending waiters, and a run that completes inside its first budget without an extension or stop decision notifies the durable managed observer exactly once on that path; and `stop` terminalizes deterministically as `cancelled`. Runtime attention now reports the initial timeout, extension count, total extended time, and remaining detach retention, and the settings summary reports the effective per-role timeout alongside the hard ceiling, the management wait window, and the detach retention window.
